...
src/
  ...
  entities/            # cross-feature domain models: types, Zod schemas, mock data, presentation (labels/formatters), repository, TanStack Query hooks
    lead/, team-member/, form/, chat/, auth/
...
- **What this phase deliberately did *not* touch**: the Leads/Forms/Chat repository implementations, `supabase/schema.sql`'s RLS policies, and the `created_by` columns (still always `NULL`). Every table's RLS policy is still `using (true)` — see the Phase 6 RLS warning above, which is **still accurate**: app-level login now gates the *React UI*, but the Supabase anon key + open policies still allow direct REST API access to every lead/form/submission/setting for anyone who has `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY`, bypassing the app entirely. Wiring `created_by = auth.uid()` and tightening RLS to per-user policies is real follow-up work, not done here, precisely so this phase couldn't accidentally break the existing Leads/Forms/Chat functionality it was told not to touch.

## Environment variables

- **`server/.env`** (git-ignored; copy from `server/.env.example`): `PORT`, `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`, `CORS_ORIGIN`. `ANTHROPIC_API_KEY` is read only here, only server-side, and is never logged. Untouched by Phase 6 or Phase 7 — no new backend env vars.
